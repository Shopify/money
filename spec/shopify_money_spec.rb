...
describe "ShopifyMoney" do
  ...
  it "raises error if added other is not compatible" do
    expect{ ShopifyMoney.new(5.00) + nil }.to raise_error(TypeError)
  end
  ...
  it "raises if divided" do
    expect { ShopifyMoney.new(55.00) / 55 }.to raise_error(RuntimeError)
  end
  ...
  it "raises when constructed with a NaN value" do
    expect { ShopifyMoney.new( 0.0 / 0) }.to raise_error(ArgumentError)
  end
  ...
  describe "from_amount quacks like RubyShopifyMoney" do
    it "accepts numeric values" do
      expect(ShopifyMoney.from_amount(1)).to eq ShopifyMoney.from_cents(1_00)
      expect(ShopifyMoney.from_amount(1.0)).to eq ShopifyMoney.from_cents(1_00)
      expect(ShopifyMoney.from_amount("1".to_d)).to eq ShopifyMoney.from_cents(1_00)
    end
    ...
    it "accepts nil values" do
      expect(ShopifyMoney.from_amount(nil)).to eq ShopifyMoney.from_cents(0)
    end

    it "accepts an optional currency parameter" do
      expect { ShopifyMoney.from_amount(1, "CAD") }.to_not raise_error
    end

    it "raises ArgumentError with unsupported argument" do
      expect { ShopifyMoney.from_amount(Object.new) }.to raise_error(ArgumentError)
    end
  end
end
